...
resource "aws_launch_template" "catalogue" {
  ...
  vpc_security_group_ids = [local.catalogue_sg_id]
  # when we run terraform apply multiple times, a new version will be created with new AMI ID . Instead of this we will updtae the version
  update_default_version = true
  # tags attached to the isnatnce
  tag_specifications {
  ...
  #tags attached to the lauch tenplate
  tags = merge(
    local.common_tags,
    {
      Name = "${local.common_name_suffix}-catalogue"
